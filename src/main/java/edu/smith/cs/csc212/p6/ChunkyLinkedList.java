...
public class ChunkyLinkedList<T> implements P6List<T> {
	...
	// O(n) because it uses remove front from fixedsizedlist
	// which is O(n)
	@Override
	 ...
	 * O(n^2)
	 * 
	 * 1) O(n) because we're using a for loop
	 * that iterates over the list and uses the methods size
	 * and remove index.
	 * 
	 * 2) Because of these two methods together, we will also have
	 *  O(2n) ( see FixedSizedList ) 
	 * 
	...
	/*
	 * O(n) because we're using methods (except for size and 
	 * fixed sized list add back) from singly
	 * linked list that take up O(n) time
	 */
	@Override
	 ...
	 * 
	 * 2) O(n) --> method add index from SLL (also remember that it 
	 * is nested within the for loop)
	 * 
	 * TOTAL O(n^2)
	 */
	@Override
	public void addIndex(T item, int index) {
		if (this.isEmpty()) {
			...
			for (FixedSizeList<T> chunk : this.chunks) {
				// calculate bounds of this chunk.
				int end = chunkNo + chunk.size();

				// Check whether the index should be in this chunk:
				if (chunkNo <= index && index <= end) {
